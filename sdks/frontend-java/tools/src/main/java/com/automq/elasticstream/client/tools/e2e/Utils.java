...
public class Utils {
    ...
    public static ByteBuffer getRecord(long seq) {
        byte[] payload = String.format("record").getBytes(StandardCharsets.UTF_8);
    ...
    public static boolean appendRecordsWithTimeout(Stream stream, long startIndex, long count, int batchSize) {
    ...
    public static boolean fetchRecords(Stream stream, long startIndex, long count, int batchSize) {
        for (long i = startIndex; i < startIndex + count; i++) {
            ...
            try {
                ...
                // log.error("Failed to fetch. Cause: " + e.toString());
                return false;
            }
    ...
    public static boolean checkFetchResult(Stream stream, int startOffset, int endOffset, int startIndex,
            int endIndex) {
        FetchResult result = null;
        try {
            result = stream.fetch(startOffset, endOffset, Integer.MAX_VALUE).get();
        } catch (InterruptedException | ExecutionException e) {
            e.printStackTrace();
            return false;
        }
        if (result.recordBatchList().size() != endIndex - startIndex) {
            return false;
        }
        int size = result.recordBatchList().size();
        for (int i = 0; i < size; i++) {
            RecordBatchWithContext recordBatch = result.recordBatchList().get(i);
            byte[] rawPayload = new byte[recordBatch.rawPayload().remaining()];
            recordBatch.rawPayload().get(rawPayload);
            String payloadStr = new String(rawPayload, StandardCharsets.UTF_8);
            if (!String.format("hello world %03d", i + startIndex).equals(payloadStr)) {
                return false;
            }
        }
        return true;
