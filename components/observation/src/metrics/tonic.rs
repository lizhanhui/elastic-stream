...
use opentelemetry::sdk::Resource as SdkResource;
...
fn transform_gauge<T: fmt::Debug + Into<TonicExemplarValue> + Into<TonicDataPointValue> + Copy>(
    gauge: &SdkGauge<T>,
) -> TonicGauge {
    TonicGauge {
        data_points: gauge
...
fn transform_exemplar<T: Into<TonicExemplarValue> + Copy>(ex: &SdkExemplar<T>) -> TonicExemplar {
    TonicExemplar {
