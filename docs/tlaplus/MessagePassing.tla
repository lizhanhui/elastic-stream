...
(*       actor. So a range_server may be delivered an AddEntryRequest from the   *)
(*       writer first, then a Read request from the 2nd client after that, *)
(*       but the range_server process the read request first, and the add second.*)
(***************************************************************************)

ReadTimeoutForRangeServer(msgs, cid, range_server) ==
    \E msg \in DOMAIN msgs :
        /\ msgs[msg] = -1
        /\ msg.range_server = range_server
        /\ msg.cid = cid
        /\ msg.type \in {ReadRequestMessage, ReadResponseMessage}

WriteTimeoutForRangeServer(msgs, cid, range_server, recovery) ==
    \E msg \in DOMAIN msgs :
        /\ msgs[msg] = -1
        /\ msg.range_server = range_server
        /\ msg.cid = cid
...
ReadTimeoutCount(cid, ensemble, recovery) ==
    IF \E b \in ensemble : ReadTimeoutForRangeServer(messages, cid, b)
    THEN Cardinality({ b \in ensemble : ReadTimeoutForRangeServer(messages, cid, b)})
    ELSE 0

ClearWriteTimeout(cid, range_servers, recovery) ==
    messages' = [m \in DOMAIN messages |-> IF /\ (m.type = AddEntryRequestMessage \/ m.type = AddEntryResponseMessage)
                                              /\ m.range_server \in range_servers
                                              /\ m.cid = cid
                                              /\ m.recovery = recovery
                                              /\ messages[m] = -1
                                           THEN 0
...
IgnoreFurtherReadResponses(msg, ensemble) ==
    messages' = [m \in DOMAIN messages |-> IF msg = m
                                           THEN 0
                                           ELSE IF /\ m.range_server \in ensemble
                                                   /\ m.cid = msg.cid
...
IsEarliestMsg(msg) ==
    ~\E msg2 \in DOMAIN messages :
        ...
        /\ msg2.cid = msg.cid
        /\ msg2.range_server = msg.range_server
