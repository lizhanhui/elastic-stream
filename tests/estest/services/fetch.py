...
class Fetch(Service):
    ...
    def __init__(self, context, num_nodes, pd, replica, count, batch_size):
        ...
        super(Fetch, self).__init__(context, num_nodes)
    ...
    def start_node(self, node):
        ...
        # node.account.ssh("mkdir -p %s" % Fetch.ROOT)
    ...
    def start_cmd(self, node):
        ...
        cmd += "java -cp /opt/*.jar  com.automq.elasticstream.client.tools.e2e.FetchTest"
        return cmd

    def pids(self, node):
        try:
            cmd = "ps -a | grep FetchTest | awk '{print $1}'"
    ...
    def kill_node(self, node):
        ...
        wait_until(lambda: not self.alive(node), timeout_sec=5, err_msg="Timed out waiting for FetchTest to be killed.")

    def stop_node(self, node):
        idx = self.idx(node)
        self.logger.info("Stopping %s node %d on %s" % (type(self).__name__, idx, node.account.hostname))
        self.signal_node(node)
        wait_until(lambda: not self.alive(node), timeout_sec=5, err_msg="Timed out waiting for FetchTest to stop.")
    ...
    def clean_node(self, node):
        self.stop_node(node)
        node.account.ssh("sudo rm -rf -- %s" % Fetch.ROOT, allow_fail=False)
