// Code generated by the FlatBuffers compiler. DO NOT EDIT.

package header

import (
	flatbuffers "github.com/google/flatbuffers/go"
)

type AppendRequest struct {
	_tab flatbuffers.Table
}

func GetRootAsAppendRequest(buf []byte, offset flatbuffers.UOffsetT) *AppendRequest {
	n := flatbuffers.GetUOffsetT(buf[offset:])
	x := &AppendRequest{}
	x.Init(buf, n+offset)
	return x
}

func GetSizePrefixedRootAsAppendRequest(buf []byte, offset flatbuffers.UOffsetT) *AppendRequest {
	n := flatbuffers.GetUOffsetT(buf[offset+flatbuffers.SizeUint32:])
	x := &AppendRequest{}
	x.Init(buf, n+offset+flatbuffers.SizeUint32)
	return x
}

func (rcv *AppendRequest) Init(buf []byte, i flatbuffers.UOffsetT) {
	rcv._tab.Bytes = buf
	rcv._tab.Pos = i
}

func (rcv *AppendRequest) Table() flatbuffers.Table {
	return rcv._tab
}

func (rcv *AppendRequest) TimeoutMs() uint32 {
	o := flatbuffers.UOffsetT(rcv._tab.Offset(4))
	if o != 0 {
		return rcv._tab.GetUint32(o + rcv._tab.Pos)
	}
	return 0
}

func (rcv *AppendRequest) MutateTimeoutMs(n uint32) bool {
	return rcv._tab.MutateUint32Slot(4, n)
}

func (rcv *AppendRequest) AppendRequests(obj *AppendInfo, j int) bool {
	o := flatbuffers.UOffsetT(rcv._tab.Offset(6))
	if o != 0 {
		x := rcv._tab.Vector(o)
		x += flatbuffers.UOffsetT(j) * 4
		x = rcv._tab.Indirect(x)
		obj.Init(rcv._tab.Bytes, x)
		return true
	}
	return false
}

func (rcv *AppendRequest) AppendRequestsLength() int {
	o := flatbuffers.UOffsetT(rcv._tab.Offset(6))
	if o != 0 {
		return rcv._tab.VectorLen(o)
	}
	return 0
}

func AppendRequestStart(builder *flatbuffers.Builder) {
	builder.StartObject(2)
}
func AppendRequestAddTimeoutMs(builder *flatbuffers.Builder, timeoutMs uint32) {
	builder.PrependUint32Slot(0, timeoutMs, 0)
}
func AppendRequestAddAppendRequests(builder *flatbuffers.Builder, appendRequests flatbuffers.UOffsetT) {
	builder.PrependUOffsetTSlot(1, flatbuffers.UOffsetT(appendRequests), 0)
}
func AppendRequestStartAppendRequestsVector(builder *flatbuffers.Builder, numElems int) flatbuffers.UOffsetT {
	return builder.StartVector(4, numElems, 4)
}
func AppendRequestEnd(builder *flatbuffers.Builder) flatbuffers.UOffsetT {
	return builder.EndObject()
}
