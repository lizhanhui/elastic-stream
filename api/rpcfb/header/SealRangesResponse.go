// Code generated by the FlatBuffers compiler. DO NOT EDIT.

package header

import (
	flatbuffers "github.com/google/flatbuffers/go"
)

type SealRangesResponse struct {
	_tab flatbuffers.Table
}

func GetRootAsSealRangesResponse(buf []byte, offset flatbuffers.UOffsetT) *SealRangesResponse {
	n := flatbuffers.GetUOffsetT(buf[offset:])
	x := &SealRangesResponse{}
	x.Init(buf, n+offset)
	return x
}

func GetSizePrefixedRootAsSealRangesResponse(buf []byte, offset flatbuffers.UOffsetT) *SealRangesResponse {
	n := flatbuffers.GetUOffsetT(buf[offset+flatbuffers.SizeUint32:])
	x := &SealRangesResponse{}
	x.Init(buf, n+offset+flatbuffers.SizeUint32)
	return x
}

func (rcv *SealRangesResponse) Init(buf []byte, i flatbuffers.UOffsetT) {
	rcv._tab.Bytes = buf
	rcv._tab.Pos = i
}

func (rcv *SealRangesResponse) Table() flatbuffers.Table {
	return rcv._tab
}

func (rcv *SealRangesResponse) ThrottleTimeMs() uint32 {
	o := flatbuffers.UOffsetT(rcv._tab.Offset(4))
	if o != 0 {
		return rcv._tab.GetUint32(o + rcv._tab.Pos)
	}
	return 0
}

func (rcv *SealRangesResponse) MutateThrottleTimeMs(n uint32) bool {
	return rcv._tab.MutateUint32Slot(4, n)
}

func (rcv *SealRangesResponse) SealResponses(obj *SealRangesResult, j int) bool {
	o := flatbuffers.UOffsetT(rcv._tab.Offset(6))
	if o != 0 {
		x := rcv._tab.Vector(o)
		x += flatbuffers.UOffsetT(j) * 4
		x = rcv._tab.Indirect(x)
		obj.Init(rcv._tab.Bytes, x)
		return true
	}
	return false
}

func (rcv *SealRangesResponse) SealResponsesLength() int {
	o := flatbuffers.UOffsetT(rcv._tab.Offset(6))
	if o != 0 {
		return rcv._tab.VectorLen(o)
	}
	return 0
}

func SealRangesResponseStart(builder *flatbuffers.Builder) {
	builder.StartObject(2)
}
func SealRangesResponseAddThrottleTimeMs(builder *flatbuffers.Builder, throttleTimeMs uint32) {
	builder.PrependUint32Slot(0, throttleTimeMs, 0)
}
func SealRangesResponseAddSealResponses(builder *flatbuffers.Builder, sealResponses flatbuffers.UOffsetT) {
	builder.PrependUOffsetTSlot(1, flatbuffers.UOffsetT(sealResponses), 0)
}
func SealRangesResponseStartSealResponsesVector(builder *flatbuffers.Builder, numElems int) flatbuffers.UOffsetT {
	return builder.StartVector(4, numElems, 4)
}
func SealRangesResponseEnd(builder *flatbuffers.Builder) flatbuffers.UOffsetT {
	return builder.EndObject()
}
